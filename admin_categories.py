...
def admin_home_cmd(message):
    if not is_admin(message.chat.id):
        return
    bot.send_message(message.chat.id, "🛠 لوحة تحكم المشرف — اختر:", reply_markup=_admin_menu())
...
def adm_cats(call):
    ...
    bot.edit_message_text(f"📂 الأقسام ({n}) — اضغط قسماً لتسميته أو حذفه:", call.message.chat.id, call.message.message_id, reply_markup=m)
...
def catmenu(call):
    ...
    m.add(types.InlineKeyboardButton("↩️ رجوع للقسمات", callback_data="adm_cats"))
